// ---------- Auto Page Detection populate.js ----------
document.addEventListener("DOMContentLoaded", () => {

  const path = window.location.pathname;
  const page = path.substring(path.lastIndexOf("/") + 1); // e.g., index.html, about.html
  ...
  if(page === "index.html" || page === ""){
    ...
    // Populate sections on Home page
    populateServices();
    populateProjects();
    populateTeam();
  ...
  function populateVMV(){
    ...
    if(vmvContainer){
      siteData.visionMissionValues.forEach(item => {
        ...
        card.style.cssText = "flex:1 1 calc(33% - 20px);border:1px solid #ddd;border-radius:8px;padding:20px;text-align:center;margin-bottom:20px;";
        card.innerHTML = `
